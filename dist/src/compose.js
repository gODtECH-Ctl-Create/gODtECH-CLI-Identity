...
function areaResize(input, outputRows, outputCols, threshold) {
...
function toAscii(mask) {
  return mask.map((row) => row.map((cell) => (cell ? "#" : " ")).join("").replace(/\s+$/u, "")).join("\n");
}
function widthOf(mask) {
...
export function composeLogo(primary, secondary, options = {}) {
  ...
  const rawSecondaryCols = Math.max(4, Math.round((rawSecondaryRows * widthOf(secondaryRaw)) / Math.max(1, secondaryRaw.length)));
  ...
  for (let y = startRow; y < Math.min(canvasRows, startRow + bandRows); y += 1) {
    for (let x = startCol; x < Math.min(targetCols, startCol + bandCols); x += 1) primaryGrid[y][x] = false;
  }
...
export function centerLine(text, targetCols = 80) {
