...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (!supportedFlags.has(value)) { console.error(`Unknown option: ${value}`); process.exit(2); }
  if (valueFlags.has(value)) {
    if (index + 1 >= args.length || args[index + 1].startsWith("-")) { console.error(`Missing value for ${value}`); process.exit(2); }
    index += 1;
  }
}
const productArg = readProductArg(args);
if (args.includes("--help")) { printHelp(); process.exit(0); }
if (!productArg) { printHelp(); process.exit(1); }
const product = getProduct(productArg);
if (!product) { console.error(`Unknown product: ${productArg}`); process.exit(2); }
if (args.includes("--quiet")) process.exit(0);
if (args.includes("--json")) { console.log(JSON.stringify(product)); process.exit(0); }
if (args.includes("--no-banner")) { console.log(`${product.symbol} ${product.description}`); process.exit(0); }
const widthValue = readFlagValue(args, "--width");
const width = widthValue === undefined ? 80 : Number(widthValue);
if (!Number.isFinite(width)) { console.error(`Invalid width: ${widthValue}`); process.exit(2); }
console.log(composeLogo("GODTECH", product.name, { targetCols: width, unicode: !args.includes("--ascii") }));
console.log("");
